...
Test Setup
    ...
    ...                You can define the ${TEST_199_SETUP} variable with a keyword for setting up this specific test.
    ...                If both are defined, ${TEST_199_SETUP} will be run after ${TEST_SETUP}.

    ${TEST_SETUP_VALUE} =        Get Variable Value    ${TEST_SETUP}
    ${TEST_199_SETUP_VALUE} =    Get Variable Value    ${TEST_199_SETUP}
    IF    $TEST_SETUP_VALUE is not None        Run Keyword    ${TEST_SETUP}
    IF    $TEST_199_SETUP_VALUE is not None    Run Keyword    ${TEST_199_SETUP}

Test Teardown
    [Documentation]    test teardown
    ...                You can define the ${TEST_TEARDOWN} variable with a keyword for tearing down all your tests.
    ...                You can define the ${TEST_199_TEARDOWN} variable with a keyword for tearing down this specific test.
    ...                If both are defined, ${TEST_TEARDOWN} will be run after ${TEST_199_TEARDOWN}.

    ${TEST_199_TEARDOWN_VALUE} =    Get Variable Value    ${TEST_199_TEARDOWN}
    ${TEST_TEARDOWN_VALUE} =        Get Variable Value    ${TEST_TEARDOWN}
    IF    $TEST_199_TEARDOWN_VALUE is not None    Run Keyword    ${TEST_199_TEARDOWN}
