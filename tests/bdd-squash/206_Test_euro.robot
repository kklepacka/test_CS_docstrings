...
Test Setup
    ...
    ...                You can define the ${TEST_206_SETUP} variable with a keyword for setting up this specific test.
    ...                If both are defined, ${TEST_206_SETUP} will be run after ${TEST_SETUP}.

    ${TEST_SETUP_VALUE} =        Get Variable Value    ${TEST_SETUP}
    ${TEST_206_SETUP_VALUE} =    Get Variable Value    ${TEST_206_SETUP}
    IF    $TEST_SETUP_VALUE is not None        Run Keyword    ${TEST_SETUP}
    IF    $TEST_206_SETUP_VALUE is not None    Run Keyword    ${TEST_206_SETUP}

Test Teardown
    [Documentation]    test teardown
    ...                You can define the ${TEST_TEARDOWN} variable with a keyword for tearing down all your tests.
    ...                You can define the ${TEST_206_TEARDOWN} variable with a keyword for tearing down this specific test.
    ...                If both are defined, ${TEST_TEARDOWN} will be run after ${TEST_206_TEARDOWN}.

    ${TEST_206_TEARDOWN_VALUE} =    Get Variable Value    ${TEST_206_TEARDOWN}
    ${TEST_TEARDOWN_VALUE} =        Get Variable Value    ${TEST_TEARDOWN}
    IF    $TEST_206_TEARDOWN_VALUE is not None    Run Keyword    ${TEST_206_TEARDOWN}
